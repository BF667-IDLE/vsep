...
from separator.ensembler import Ensembler
from config import (
    UVR_PUBLIC_REPO_URL,
    UVR_VIP_REPO_URL,
...
class Separator:
    ...
    def download_file_if_not_exists(self, url, output_path):
        ...
        Uses optimized settings for faster downloads: larger chunk size, connection pooling, and resume support.
        """
        ...
        self.logger.debug(f"Downloading file from {url} to {output_path} with timeout {DOWNLOAD_TIMEOUT}s")
        ...
        if os.path.exists(output_path):
            ...
            self.logger.debug(f"Resuming download from byte {first_byte}")
        
        try:
            response = session.get(url, stream=True, timeout=DOWNLOAD_TIMEOUT, headers=headers)
            response.raise_for_status()
            
            # Get total size for progress bar
            total_size = int(response.headers.get('content-length', 0))
            if total_size == 0:
                total_size = first_byte  # Unknown size, use current position
            
            # Use larger chunk size for faster downloads (256KB instead of 8KB)
            chunk_size = DOWNLOAD_CHUNK_SIZE  # 256KB
            ...
            with open(output_path, mode) as f:
                with tqdm(
                    ...
                    unit_scale=True,
                    unit_divisor=1024,
                    desc=os.path.basename(output_path)[:50]
                ) as progress_bar:
    ...
    def download_model_files(self, model_filename):
        ...
        for model_type, models in supported_model_files_grouped.items():
            ...
            for model_friendly_name, model_info in models.items():
                ...
                model_repo_url = get_repo_url(is_vip=self.model_is_uvr_vip)
                ...
                if model_info["filename"] == model_filename or model_filename in model_info["download_files"]:
                    ...
                    for file_to_download in model_info["download_files"]:
                        ...
                        if file_to_download.startswith("http"):
                            ...
                            download_tasks.append((file_to_download, download_path))
                            continue
                        ...
                        if model_type == "MDXC" and file_to_download.endswith(".yaml"):
                            ...
                            yaml_url = get_mdx_yaml_url(file_to_download)
                            download_tasks.append((yaml_url, download_path))
                            continue

                        # For regular model files, use primary repo URL
                        download_url = f"{model_repo_url}/{file_to_download}"
                        download_tasks.append((download_url, download_path))

                    # Download files in parallel (up to 4 concurrent downloads)
                    self._download_files_parallel(download_tasks)
    ...
    def _download_files_parallel(self, download_tasks: List[Tuple[str, str]], max_workers: int = None):
        """
        Download multiple files in parallel using ThreadPoolExecutor.
        
        Args:
            download_tasks: List of (url, output_path) tuples
            max_workers: Maximum number of parallel download threads (default: from config)
        """
        if max_workers is None:
            max_workers = MAX_DOWNLOAD_WORKERS
        
        self.logger.info(f"Downloading {len(download_tasks)} files in parallel with {max_workers} workers...")
        
        failed_downloads = []
        
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_url = {
                executor.submit(self.download_file_if_not_exists, url, path): (url, path)
            ...
            for future in as_completed(future_to_url):
                ...
                try:
                    future.result()
                    self.logger.debug(f"Successfully downloaded: {os.path.basename(path)}")
                except Exception as e:
                    self.logger.error(f"Failed to download {url}: {e}")
                    failed_downloads.append((url, path, e))
        
        if failed_downloads:
            # Retry failed downloads sequentially with fallback URLs
            for url, path, error in failed_downloads:
                if "github.com" in url and "audio-separator" not in url:
                    # Try fallback to audio-separator repo
                    filename = os.path.basename(path)
                    fallback_url = get_fallback_url(filename)
                    self.logger.info(f"Retrying with fallback URL: {fallback_url}")
                    try:
                        self.download_file_if_not_exists(fallback_url, path)
                        self.logger.debug(f"Successfully downloaded from fallback: {os.path.basename(path)}")
                    except Exception as fallback_error:
                        self.logger.error(f"Fallback also failed for {filename}: {fallback_error}")
                        raise RuntimeError(f"Failed to download {filename} from all sources") from fallback_error
                else:
                    raise RuntimeError(f"Failed to download {os.path.basename(path)}") from error
    ...
    def load_model_data_using_hash(self, model_path):
        ...
        vr_model_data_path = os.path.join(self.model_file_dir, VR_MODEL_DATA_FILENAME)
        self.logger.debug(f"VR model data path set to {vr_model_data_path}")
        self.download_file_if_not_exists(UVR_VR_MODEL_DATA_URL, vr_model_data_path)

        mdx_model_data_path = os.path.join(self.model_file_dir, MDX_MODEL_DATA_FILENAME)
        self.logger.debug(f"MDX model data path set to {mdx_model_data_path}")
        self.download_file_if_not_exists(UVR_MDX_MODEL_DATA_URL, mdx_model_data_path)
    ...
    def _separate_file(self, audio_file_path, custom_output_names=None):
        ...
        if self.chunk_duration is not None:
            ...
            from separator.audio_chunking import AudioChunker
